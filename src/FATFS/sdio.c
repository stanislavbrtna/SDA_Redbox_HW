...
/* SD card is connected to SPI3, PC10-11-12 a PD2 */
#define SPI_SD SPI3
static void spi_init(void)
{
	...
	gpio.Pin = GPIO_PIN_10 | GPIO_PIN_11 | GPIO_PIN_12; //piny SPI periferie
	...
	printf("spi_dbg: gpio ok\n");
	init=1;

}
#define spi_cs_low() do { GPIOD->BSRR = (uint32_t)GPIO_PIN_2 << 16; } while (0)
#define spi_cs_high() do { GPIOD->BSRR = GPIO_PIN_2; } while (0)
...
static void spi_set_speed(enum sd_speed speed)
{
	...
	if (speed == SD_SPEED_400KHZ){
		prescaler = SPI_BAUDRATEPRESCALER_256; //orig 256
		printf("SD SPEED: 400khz\n");
	} else if (speed == SD_SPEED_25MHZ) {
		prescaler = SPI_BAUDRATEPRESCALER_2; //orig 4
		printf("SD SPEED: 25Mhz\n");
	}
	...
	spi_handle.Init.Direction   = SPI_DIRECTION_2LINES; //SPI_Direction_2Lines_FullDuplex;
	...
	printf("spi_dbg: hal init\n");

	if(HAL_SPI_Init(&spi_handle)== HAL_OK )
		printf("spi_dbg: hal init ok\n");
	else
		printf("spi_dbg: hal init FAIL\n");
    //AFIO->MAPR &= ~ AFIO_MAPR_SPI1_REMAP; //dodáno remapování
...
static u8 spi_txrx(u8 data)
{
	...
	HAL_SPI_TransmitReceive(&spi_handle, &data, &retval, sizeof(data), 100);
	//HAL_SPI_TransmitReceive_IT(&spi_handle, &data, &retval, sizeof(data));
	...
	if (hw->capabilities & CAP_VER2_00) {
		...
		if (r & 0xfe) {
			...
			printf("%i.%iV, CCS:%li, power up status:%li\n",
					(13+i)/10, (13+i)%10,
					r3>>30 & 1, r3>>31);
			// power up status should be 1 here, since we're finished initializing, but it's not. WHY?
...
int hwif_init(hwif* hw)
{
  ...
  //ignorujem, jináč nefunguje reinit
  //ovšem pozor, nutno mít k tomu uzpůsobené inicializační fce pro hw
	//if (hw->initialized)
	//	return 0;

	printf("fatfs volá hw init\n");
	spi_init();
